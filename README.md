...
- 抢课逻辑在原项目基础上做过修正：
  ...
  - 原版末尾的 `query_result()` 没有异常保护，最后一步出错会让程序带着 traceback 崩掉；
    现在只提示，不影响前面已经抢到的结果。
  - 原版把「响应里出现『添加选课志愿成功』」直接当成抢到了；但学校受理 ≠ 真的落库
    （同类项目记录过这种假成功）。现在改成：提交回执只算候选，必须回查「已选课程」列表、
    按教学班 ID 精确匹配后才算抢到；查不到就留在待抢列表继续抢。已选列表里虽然有课、
    但一个可用的 ID 字段都没有时，会降级为信任提交回执并明确告警（响应原文在 `logs/` 里）。
  - 新增 `logs/` 响应原文落盘（已 gitignore），方便用真实响应校准判定规则。
